class Movie < ApplicationRecord
  ...
  # - title: must be present; must be unique in combination with year (2 movies with same title won't come out in the same year)
  validates :title, :presence => true, :uniqueness => {:scope => :year}#, :uniqueness => true (this would be good for username)
